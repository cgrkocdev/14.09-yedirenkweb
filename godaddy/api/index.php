...
if ($path === '/payment/albaraka/initialize') {
    ...
    $month=(int)substr($expiry,2,2);$donorValid=text($b['firstName']??'',80)&&text($b['lastName']??'',80)&&text($b['phone']??'',30)&&filter_var(text($b['email']??'',254),FILTER_VALIDATE_EMAIL)&&text($b['description']??'',500)&&text($b['campaign']??'',500)&&is_array($b['items']??null)&&count($b['items'])>0;
    if (!preg_match('/^\d{16,19}$/',$cardNo)||!preg_match('/^\d{3,4}$/',$cvv)||!preg_match('/^\d{4}$/',$expiry)||$month<1||$month>12||!$holder||!$donorValid||$amount<1||$amount>1000000000||($b['consent']??false)!==true) fail('Kart ve bağış bilgilerini kontrol edin.');
...
if ($path === '/payment/albaraka/callback') {
    ...
    if((int)($_SERVER['CONTENT_LENGTH']??0)>30000)paymentPage(false,'Banka yanıtı çok büyük.');
    ...
    ensurePaymentTable($db);$q=$db->prepare("SELECT * FROM payment_transactions WHERE order_id=? AND status='3d_pending' LIMIT 1");$q->execute([$order]);$row=$q->fetch();if(!$row||nowMs()-(int)$row['created_at_ms']>3600000)paymentPage(false,'Sipariş bulunamadı, daha önce işlendi veya süresi doldu.',$order);$pending=json_decode($row['payload_json'],true);
    $returnedAmount=$get('Amount');$returnedMerchant=$get('MerchantId');$returnedType=$get('TranType');if(($returnedAmount!==''&&(int)$returnedAmount!==(int)$row['amount'])||($returnedMerchant!==''&&!hash_equals($c['merchantNo'],$returnedMerchant))||($returnedType!==''&&strcasecmp($returnedType,'Sale')!==0))paymentPage(false,'Banka dönüşündeki sipariş bilgileri eşleşmiyor.',$order);
    $claim=$db->prepare("UPDATE payment_transactions SET status='processing' WHERE order_id=? AND status='3d_pending'");$claim->execute([$order]);if($claim->rowCount()!==1)paymentPage(false,'Sipariş daha önce işleme alınmış.',$order);
    $saleMac=albarakaHash($c['merchantNo'].$c['terminalNo'].$secureId.$cavv.$eci.$mdStatus.$c['encKey']);$sale=['ApiType'=>'JSON','ApiVersion'=>'V100','MerchantNo'=>$c['merchantNo'],'TerminalNo'=>$c['terminalNo'],'PaymentInstrumentType'=>'CARD','CipheredData'=>null,'DealerData'=>null,'PaymentFacilitatorData'=>null,'AdditionalInfoData'=>null,'CardInformationData'=>null,'IsEncrypted'=>'N','IsTDSecureMerchant'=>'Y','IsMailOrder'=>'N','IsRecurring'=>null,'ThreeDSecureData'=>['SecureTransactionId'=>$secureId,'CavvData'=>$cavv,'Eci'=>$eci,'MdStatus'=>1,'MD'=>$md],'MAC'=>$saleMac,'MACParams'=>'MerchantNo:TerminalNo:SecureTransactionId:CavvData:Eci:MdStatus','Amount'=>(int)$row['amount'],'CurrencyCode'=>'TL','PointAmount'=>0,'OrderId'=>$order,'InstallmentCount'=>0,'InstallmentType'=>'N','KOICode'=>null,'MerchantMessageData'=>null];
    $curl=curl_init($c['serviceUrl'].'/Sale');curl_setopt_array($curl,[CURLOPT_RETURNTRANSFER=>true,CURLOPT_POST=>true,CURLOPT_POSTFIELDS=>json_encode($sale),CURLOPT_CONNECTTIMEOUT=>8,CURLOPT_TIMEOUT=>25,CURLOPT_HTTPHEADER=>['Content-Type: application/json; charset=UTF-8','Accept: application/json','X-MERCHANT-ID: '.$c['merchantNo'],'X-TERMINAL-ID: '.$c['terminalNo'],'X-POSNET-ID: '.$c['posnetId'],'X-CORRELATION-ID: '.substr($order.'S',0,20)]]);$raw=curl_exec($curl);$status=(int)curl_getinfo($curl,CURLINFO_RESPONSE_CODE);curl_close($curl);$result=json_decode((string)$raw,true);if($status<200||$status>=300||!is_array($result)){$db->prepare("UPDATE payment_transactions SET status='review_required' WHERE order_id=?")->execute([$order]);paymentPage(false,'Satış sonucu kesinleşmedi. Lütfen sipariş numarasıyla derneğe başvurun.',$order);}$code=(string)($result['ServiceResponseData']['ResponseCode']??'');if(!in_array($code,['00','0000'],true)){$db->prepare("UPDATE payment_transactions SET status='failed' WHERE order_id=?")->execute([$order]);paymentPage(false,(string)($result['ServiceResponseData']['ResponseDescription']??('Banka hata kodu: '.$code)),$order);}
